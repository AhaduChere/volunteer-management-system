...
export default function VolunteerSummaryPage({ initialVolunteers }: { initialVolunteers: Volunteer[] }) {
  ...
  const filteredVolunteers = initialVolunteers.filter((v) => {
    const matchesFilter =
      ...
      v.status === filter;
      var name:string  = v.firstName + ' ' + v.lastName;
          ...
          {filteredVolunteers.map((vol) => (
            <tr key={vol.id} className="border-b border-zinc-300 hover:bg-[#1c2355]">
              ...
              <td className="p-3 space-x-2">
                <button
                  onClick={() => alert(`Viewing matches for ${vol.firstName + ' ' + vol.lastName}`)}
